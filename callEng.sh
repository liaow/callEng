...
callLogFile=${LOG_PATH}/call_${startTimeStr}.log
...
usage ()
{
        ...
        echo "  e,  eparchy_code,    必选,  需要处理的地州编码(4位)[0表示所有地州,也可以传多个以半角冒号隔开],参考其他说明"
        echo "  c,  sett_month,      必选,  需要处理的月份[-1:上月;0:本月;其他(yyyymm):指定账期]"
        echo "  r,  rule_type_code,  必选,  类型,如4100[0表示所有业务,也可以传多个以半角冒号隔开],参考其他说明"
...
if [ "$DB_ON" = "Y" ] ; then
    ...
    Select Replace(Wmsys.Wm_Concat(Data_Id),',',':') ruleType From Rwd_Core_Static a Where a.Type_Id = 'RWD_RULE_TYPE_CODE';
    EXIT;  
EOF`
fi
...
echo `date +"%Y-%m-%d %H:%M:%S"`" current RULE_TYPE_CODE : "$RULE_TYPE_CODE >>$callLogFile

#从数据库读地州编码
if [ "$DB_ON" = "Y" ] ; then
    TMP_EPARCHY_CODE=`sqlplus -silent $CONNSTR <<EOF  
    SET PAGESIZE 0 FEEDBACK OFF VERIFY OFF HEADING OFF ECHO OFF  
    Select Replace(Wmsys.Wm_Concat(Area_Code),',',':') areaCode From Rwd_Core_Area a Where a.Area_Level = 20;  
    EXIT;  
EOF`
fi
echo `date +"%Y-%m-%d %H:%M:%S"`" EPARCHY_CODE equals ${EPARCHY_CODE} ,use database =${DB_ON} , TMP_EPARCHY_CODE : "$TMP_EPARCHY_CODE >>$callLogFile

if [ "$EPARCHY_CODE" = "0" ]; then
...
echo `date +"%Y-%m-%d %H:%M:%S"`" current EPARCHY_CODE : "$EPARCHY_CODE >>$callLogFile
...
for ruleTypeCode in ${arrRuleTypeCodes[@]}; do
        for eparchyCode in ${arrEparchyCodes[@]}; do
                #程序详细日志
                programLogFile=${LOG_PATH}/${PROGRAM_NAME}_${eparchyCode}_${ruleTypeCode}_${startTimeStr}.log
                ...
                sleep 10
                echo `date +"%Y-%m-%d %H:%M:%S"`" begin execute executeEng.sh,param[sett_month|eparchy_code|rule_type_code|node_id|max_active|config_file|other_params]" >> $callLogFile
                echo `date +"%Y-%m-%d %H:%M:%S"`"                                 ["$SETT_MONTH"|"$eparchyCode"|"$ruleTypeCode"|"$NODE_ID"|"$MAX_ACTIVE"|"$CONFIG_FILE"|"$OTHER_PARAMS"]" >> $callLogFile
                #程序调用开始
                echo `date +"%Y-%m-%d %H:%M:%S"`" ./executeEng.sh "$PROGRAM_NAME" -i"${CONFIG_FILE}" -c"${SETT_MONTH}" -e"${eparchyCode}" -r"${ruleTypeCode}" "${NODE_PARAM}" -"${OTHER_PARAMS} >> $callLogFile
                ./executeEng.sh $PROGRAM_NAME -i${CONFIG_FILE} -c${SETT_MONTH} -e${eparchyCode} -r${ruleTypeCode} ${NODE_PARAM} -${OTHER_PARAMS} >> $programLogFile &
        done
done
